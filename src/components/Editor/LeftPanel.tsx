...
function uploadViaFormData(url: string, file: File, headers?: Record<string, string>) {
...
async function loadDirectUploadCapability() {
    try {
        const response = await fetch(`${MVP_API_BASE_URL}/upload-init`, {
            cache: "no-store",
        });
        if (!response.ok) {
        ...
        return {
            ...
            transport: payload.transport === "backend" ? payload.transport : null,
        ...
        return {
            available: false,
            transport: null as MvpDirectUploadTransport,
            directUploadUrl: "",
...
export default function LeftPanel({
    ...
    const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const uploadCapability = await loadDirectUploadCapability();
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (directUploadEnabled && resolvedDirectUploadTransport === "backend" && resolvedDirectUploadUrl) {
                    ...
                    payload = await uploadViaFormData(backendUploadUrl, file, backendUploadHeaders);
                } else {
        ...
        if (uploadedItems.length > 0) {
            ...
            onInputReady?.(uploadedItems[uploadedItems.length - 1].sourceName);
            setStatusText(
