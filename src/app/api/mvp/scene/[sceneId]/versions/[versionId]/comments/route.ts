...
export async function GET(request: NextRequest, context: { params: Promise<{ sceneId: string; versionId: string }> }) {
    ...
    return proxyToBackend(request, `scene/${sceneId}/versions/${versionId}/comments`);
}

export async function POST(request: NextRequest, context: { params: Promise<{ sceneId: string; versionId: string }> }) {
    const { sceneId, versionId } = await context.params;
    return proxyToBackend(request, `scene/${sceneId}/versions/${versionId}/comments`);
}
